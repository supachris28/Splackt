...
def ground(tagged_text, base_date):
    ...
    for timex in timex_found:
        ...
        # Relative dates
            ...
            timex_val = str(base_date + datetime.timedelta(days=-1))
        ...
        # Last, this, next week.
            ...
            newdate = base_date - datetime.timedelta(weeks=1)
            timex_val = str(newdate.year) + "W" + ("0" + str(newdate.isocalendar()[1]))[:2]
        elif re.match(r'this week', timex, re.IGNORECASE):
            year = (base_date - datetime.timedelta(base_date.weekday())).year
            week = (base_date - datetime.timedelta(base_date.weekday())).isocalendar()[1]
            timex_val = str(year) + 'W' + ("0" + str(week))[:2]
        ...
        elif re.match(regxp6, timex, re.IGNORECASE):
            month = hashmonths[timex.split()[0]]
            year = timex.split()[1]
            timex_val = year + '-' + str(month)
        ...
        # Month in the following year.
            ...
            timex_val = str(base_date.year + 1) + '-' + str(month)
        # clever month
        elif re.match(r'(in|for) ' + months, timex, re.IGNORECASE):
            month = hashmonths[timex.split()[1]]
            thisyear = datetime.datetime.now().year
            if month > datetime.datetime.now().month:
                thisyear = thisyear - 1
            timex_val = str(thisyear) + '-' + str(month)
            ...
            timex_val = str((base_date + datetime.timedelta(days=-offset)).date())
        elif re.match(r'\d+ days? (later|after)', timex, re.IGNORECASE):
            offset = int(re.split(r'\s', timex)[0])
            timex_val = str((base_date + datetime.timedelta(days=offset)).date())
        elif re.match(r'\d+ weeks? ?(ago|earlier|before|)', timex, re.IGNORECASE):
            offset = int(re.split(r'\s', timex)[0])
            year = (base_date + datetime.timedelta(weeks=-offset)).year
            week = (base_date + \
                            datetime.timedelta(weeks=-offset)).isocalendar()[1]
            timex_val = str(year) + 'W' + str(week)
        elif re.match(r'\d+ weeks? (later|after)', timex, re.IGNORECASE):
            offset = int(re.split(r'\s', timex)[0])
            year = (base_date + datetime.timedelta(weeks=+offset)).year
            week = (base_date + datetime.timedelta(weeks=+offset)).isocalendar()[1]
            timex_val = str(year) + 'W' + str(week)
